...
breed [Hajjis hajji]
breed [walls wall]
breed [nHajjis nHajji] ;Represents Aggressive hajjis, whose speed is relatively more and head directly towards Jamarah to perform Ramy
...
to setup-Jamarat
 ;bitmap:copy-to-pcolors (bitmap:import "jamarah1.png") true ;760 152
 import-pcolors "model back ground3[979].png"
end

to setup-walls
  ask (patches with [pcolor = white or pycor = 0  or pycor = 299])
  [
    sprout-walls 1
    [
      set shape "square"
...
to go
   move-Hajjis
  ifelse (count Hajjis) > 0
  [
    if total-Hajjis < Total-Hajjis-To-Create
    [add-Hajjis]
    ;if Hajjis-In-Jamarat > 0
    if ticks mod 100 = 0 [ ; every File-Write-Frequency
    let Waiting-Hajjis (count turtles with [in-queue = true])
    let Hajjis-Doing-Ramy (count turtles with [wait-time > 0])
    ]
    tick
  ]
  [
...
to move-Hajjis
  ask Hajjis [
    ...
    if xcor >= 1310 and ycor >= 235 [
     if not ramy-done [set Hajjis-Gone-Without-Ramy Hajjis-Gone-Without-Ramy + 1]
     set Hajjis-Exited-From-Jamarat Hajjis-Exited-From-Jamarat + 1
     set Hajjis-In-Jamarat Hajjis-In-Jamarat - 1
      die]

    if xcor <= 1125 and xcor >= 1087 and ycor <= 112  [
     if not ramy-done [set Hajjis-Gone-Without-Ramy Hajjis-Gone-Without-Ramy + 1]
     set Hajjis-Exited-From-Jamarat Hajjis-Exited-From-Jamarat + 1
     set Hajjis-In-Jamarat Hajjis-In-Jamarat - 1
      die]
    ...
    if xcor > 920 [
    if aimed-jamarah > 0 and aimed-jamarah > 1
    [move-to-exit1]
    if aimed-jamarah > 0 and aimed-jamarah < 2
    [move-to-exit2]
    if aimed-jamarah < 2 and aimed-jamarah < 1
    [move-to-exit1]]
    ]
end
...
to move-forward
   ...
   if (ycor <= 2 or ycor >= 297 ) and ramy-done
  [
    set heading 90
    jump speed
    stop
...
exits-width
exits-width
"defult" "increase by 5 wXd" "increase by 6 wXd"
0
...
time-of-scenario
time-of-scenario
"time zero" "peek Time" "end of the day"
0
...
260
439
percentage-of-existing-leaders
percentage-of-existing-leaders
0
100
51.0
